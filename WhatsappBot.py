...
#driver = webdriver.Chrome(executable_path=r'C:\Users\Erastus\WhatsappBOT-master\chromedriver.exe')
...
width = 1000
height = 700
canvas1 = tk.Canvas(master, width=width, height=height, relief='raised', bg='black')
canvas1.pack()
...
# put gif image on canvas
# pic's upper left corner (NW) on the canvas is at x=50 y=10
canvas1.create_image(width / 4, height / 2, image=gif1)
...
def blueSelection(event=None):
    l1 = tk.Label(master,
                  text="Enter Your Message Here ", bg="blue")
    l2 = tk.Label(master,
                  text="Number of Message(s) To Send", bg="blue")
    l3 = tk.Label(master,
                  text="Enter the Phone Number ", bg="blue")

    canvas1.create_window(600, 150, window=l1)
    canvas1.create_window(615, 190, window=l2)
    canvas1.create_window(600, 230, window=l3)
	#m1.after(1200, m1.destroy)
    ...
    canvas1.create_window(900, 150, window=e1)
    canvas1.create_window(900, 190, window=e2)
    canvas1.create_window(900, 230, window=e3)
	#canvas1.destroy(11)
	#canvas1.destroy(12)canvas1.destroy13,canvas1.destroy14)
    ...
    def Driver():
        ...
        if len(str(phone_number)) != 12:
            ...
            driver = webdriver.Chrome(executable_path=r'C:\Users\Erastus\WhatsappBOT-master\chromedriver.exe', options=options)
    ...
    canvas1.create_window(650, 280, window=c1)
    ...
    def Driver():
        ...
        if hour >= 24:
            ...
            if hour == now.hour and minutes == now.minute:
                ...
                driver.get("http://web.whatsapp.com")
                sleep(15)  # wait time to scan the code in second
            driver = webdriver.Chrome(executable_path=r'C:\Users\Erastus\WhatsappBOT-master\cromedriver.exe', options=options)
            driver.get("http://web.whatsapp.com")
            sleep(15)  # wait time to scan the code in second
